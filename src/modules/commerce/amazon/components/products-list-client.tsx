...
import { ProductDiagnosisClient } from "@/modules/commerce/amazon/components/product-diagnosis-client";
...
export function ProductsListClient() {
  ...
  const { data, pending, refreshing } = useCommerceQuery<{
    ...
    `/api/commerce/amazon?view=products&range=${encodeURIComponent(range)}`
  );
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3">
        {products.map((p, i) => {
          const open = expandedId === p.productId;
          return (
            <div
              key={p.productId}
              id={`product-row-${p.productId}`}
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0">
                      ...
                      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-[13px] text-zinc-600">
                        <span>
                          访问量 {Math.round(p.sessions.current)} (
                          {formatPct(p.sessions.deltaPct)})
                        </span>
                        <span>
                        ...
                        <span>
                          订单 {Math.round(p.orders.current)} (
                          {formatPct(p.orders.deltaPct)})
                        </span>
                      </div>
                    </div>
                    <div className="shrink-0 text-left sm:text-right">
                      <p className="text-[14px] font-medium text-zinc-800">
                        销售额 {formatMoney(p.sales.current)}{" "}
